...
from .muon_utils import to_local, momentum_update_pre_orthogonalize, muonh_update
...
class MuonH(Optimizer):
    ...
    @torch.no_grad()
    def step_muonh(self):
        assert self.param_groups[1]["algorithm"] == "muonh"
        ...
        for module_name, param in self.param_groups[1]["named_params"].items():
            ...
            if 'attention' in module_name:
                ...
                # AdamW align w. 1B 0.24 * sqrt(1024)
                C, R = 7.34, n_out ** 0.5
                U_full = muonh_update(U_full, C * R, lr)
                W_full.sub_(U_full)
                W_full.mul_(R / torch.linalg.norm(W_full.to(torch.float32), ord='fro', dim=(-2, -1), keepdim=True).clamp_min_(eps))
                W_full = W_full.view(-1, W_full.shape[-1])
                ...
                # AdamW align w. 1B 0.24 * sqrt(1024)
                C, R = 7.34, n_out ** 0.5             
                U_local = muonh_update(U_local, C * R, lr)  
                W_local.sub_(U_local)
                W_local.mul_(R / torch.linalg.norm(W_local.to(torch.float32), ord='fro', dim=(-2, -1), keepdim=True).clamp_min(eps))
                
                dist.barrier(group=self._process_group)
            elif 'router' in module_name:
                # we left rt for head-wise process for attention
                if "momentum" not in self.state[param]:
                    self.state[param]["momentum"] = torch.zeros_like(param)
                momentum = self.state[param]["momentum"]
                W_local, G_local, M_local = to_local(param), to_local(param.grad), to_local(momentum)
...
class MuonHOptimizers(OptimizersContainer):
    def __init__(self, model_parts, world_mesh, optimizer_kwargs,):
        ...
        assert name in ["MuonH"]

        for model in self.model_parts:
            params = []
            muonh_params, adamw_params = [], []
            muonh_named_params, adamw_named_params = defaultdict(DTensor), defaultdict(DTensor)

            for n, p in model.named_parameters():
                assert p.requires_grad 
                if 'output.weight' in n or 'norm.weight' in n or 'tok_embeddings.weight' in n:
                    adamw_params.append(p)
                    adamw_named_params[n] = p
                elif 'mlp' in n:
                    p.ep_per_rank = ep_per_rank
                    muonh_params.append(p)
                    muonh_named_params[n] = p
                else:
            ...
            params.append({"params": muonh_params, "named_params": muonh_named_params, "algorithm": "muonh"})
            # params.append({"params": adamwh_params, "named_params": adamwh_named_params, "algorithm": "adamwh"})
        
            self.optimizers.append(MuonH(params, world_mesh, **optimizer_kwargs))
...
def build_mh_optimizers(
    ...
    return MuonHOptimizers(model_parts, world_mesh, optimizer_kwargs)
