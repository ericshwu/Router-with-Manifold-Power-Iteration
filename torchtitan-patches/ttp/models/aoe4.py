...
class MoEMbs(MoE):
    def __init__(self, args):
        ...
        del self.router
        self.router = nn.Parameter(torch.zeros(size=(args.moe_num_experts, args.hidden_size), dtype=torch.float32))
    ...
    def forward(self, x: torch.Tensor):
        ...
        w1 = self.experts.mlp.w1.view(self.args.moe_num_experts, self.args.ffn_hidden_size, self.args.hidden_size)     
        phi = ((self.router.unsqueeze(1) @ w1.transpose(1, 2)) @ w1).squeeze(1)
        phi = F.normalize(phi, p=2, dim=-1)

        # R = C * (self.args.moe_num_experts ** -0.5)
        R = 0.125

        logits = R * x.view(-1, x.shape[-1]) @ phi.T
        logits -= logits.max(-1, keepdim=True)[0]
        scores = torch.exp(logits.to(torch.float32))

        expert_weights, top_experts = torch.topk(scores, k=self.top_k, dim=-1)
...
class MoEBlock(nn.Module):
    def __init__(self, layer_id: int, model_args: TransformerModelArgs):
        ...
        args = Arguments(
            ...
            fp16=False,
            bf16=False,
            # init_method=partial(customized_paramters_init, weight_init_std=model_args.weight_init_std),
...
class TransformerAoE4(nn.Module, ModelProtocol):
    ...
    def init_weights(self, buffer_device: torch.device | None = None):
        ...
        for layer in self.layers.values():
            ...
            assert weight_init_fn in ["default", "spectral-mup-init"]

            if weight_init_fn == "default":
            ...
            elif weight_init_fn == "spectral-mup-init":
                raise NotImplementedError
            else:
                raise NotImplementedError
            
            nn.init.trunc_normal_(layer.moe.router, mean=0., std=weight_init_std, a=-cutoff, b=cutoff)
    ...
    def init_weights(self, buffer_device: torch.device | None = None):
        ...
        for layer in self.layers.values():
            ...
            if weight_init_fn == "adamw":
                ...
                nn.init.normal_(layer.moe.router, mean=0., std=1. / (self.model_args.dim ** 0.5))
            elif weight_init_fn == "adamwh":
                _ = layer.attention
                for linear in (_.wq, _.wk, _.wv, _.wo):
                    nn.init.normal_(linear.weight, mean=0., std=1. / (self.model_args.dim ** 0.5))
